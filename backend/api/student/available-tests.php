...
try {
    ...
    Response::logRequest('student/available-tests', 'GET', $user['id']);
    
    Response::success('Available tests retrieved', $tests);
    
} catch (Exception $e) {
    error_log('Available tests error: ' . $e->getMessage());
    Response::serverError('Failed to get available tests: ' . $e->getMessage());
}
